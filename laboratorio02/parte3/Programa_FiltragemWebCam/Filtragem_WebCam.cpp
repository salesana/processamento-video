...
int main()
{
	...
	VideoWriter output("output.png", VideoWriter::fourcc('M', 'J', 'P', 'G'), fps, frame_size);
	
   while (vid_capture.isOpened())
    {
        // Initialize frame matrix to store frames
        Mat frame; Mat median;
        ...
        {
        	median = frame.clone();
            // aplica filtro median_blur 5x5 nos frames
		    for ( int i = 1; i < 3; i = i + 2 )
	    {
		medianBlur ( frame, median, i );
	    }
            //mostra o frame filtrado
            imshow("Frame_filtrado", median);
